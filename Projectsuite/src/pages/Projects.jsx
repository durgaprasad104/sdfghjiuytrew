        ...
        const fetchProjects = async () => {
            try {
                ...
                if (dbProjects && dbProjects.length > 0) {
                    ...
                    // Combine static and db projects, or just use DB projects.
                    // Let's prepend DB projects to static ones for now so user sees both
                    setProjectsList([...normalizedProjects, ...staticProjects]);
                }
            } catch (error) {
                console.error("Error fetching projects:", error);
                // Fallback to staticProjects is already set in initial state
            } finally {
                setLoading(false);
            }
        };
